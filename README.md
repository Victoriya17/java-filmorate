...
* Среда выполнения: Java 21 (дистрибутив Amazon Corretto)
* Инструмент сборки: Maven 3.9 или выше
* Порт по умолчанию: 8080 (должен быть свободен перед запуском)
...
2. Перейдите в корневую директорию проекта:
   cd filmorate
3. Скомпилируйте исходный код и соберите исполняемый jar-файл (без запуска тестов):
   mvn clean package -DskipTests
4. Запустите собранное Spring Boot приложение:
   java -jar target/filmorate-0.0.1-SNAPSHOT.jar

После этого сервис станет доступен локально по адресу http://localhost:8080.
